...
// third-party (Chart.js & React-Chartjs-2 as requested)
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
// High-fidelity fallback simulated data matching exactly the backend structure
const fallbackData = {
  active_counts: {
    BEX: 12,
    ADI: 8,
...
const DefaultDashboard = () => {
  ...
  // Fetch Dashboard data from the secure Django analytic API
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Only append agent_id parameter if user has permission (ADMIN or CHEF_SERVICE)
      ...
      // Process result and ensure filters metadata is present
      const processed = {
        ...response,
      ...
      // Fallback sandbox mode to ensure perfect local experience
  ...
  // 1. Grouped Bar Chart (Dossiers processed)
  const barChartData = {
  ...
  // 3. Line Chart (Weekly Delay Trend)
  const lineChartData = {
  ...
  // Check roles for display constraints
    ...
    <Grid container spacing={gridSpacing}>
        ...
        <PremiumCard>
          <CardContent sx={{ py: '16px !important' }}>
            <Grid container spacing={2} alignItems="center">
              ...
              {/* Agent Selector - Hidden completely for agents (Row-level security enforcement) */}
              {!isAgent && (
                <Grid item xs={12} sm={3}>
                  <FormControl fullWidth size="small">
      ...
      <Grid item xs={12}>
        <Grid container spacing={gridSpacing}>
          ...
          {/* Card 3: Validation Rate (SLA compliance) */}
          <Grid item lg={3} sm={6} xs={12}>
            <PremiumCard>
              <CardContent sx={{ p: 3, position: 'relative' }}>
