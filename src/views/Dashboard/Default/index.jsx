...
const Default = () => {
  ...
  const { user } = useAuth();
  const [stats, setStats] = React.useState({ total_hebdo: 0, en_cours: 0 });

  React.useEffect(() => {
    const fetchStats = async () => {
      try {
        const data = await api.get('/api/transit/bex/rapport-hebdo/');
        setStats(data);
      } catch (err) {
    ...
    if (user?.role === 'CHEF' || user?.role === 'CHEF_SERVICE') {
      fetchStats();
    }
  }, [user]);
    ...
    <Grid container spacing={gridSpacing}>
      <Grid item xs={12}>
        ...
        <Typography variant="subtitle1" color="textSecondary" mb={2}>
          {(user?.role === 'CHEF' || user?.role === 'CHEF_SERVICE')
            ? "Tableau de bord de supervision - Pilotage de l'activité hebdomadaire."
            : "Voici un aperçu de l'activité du suivi documentaire Laborex Transit."}
        </Typography>
      </Grid>
      <Grid item xs={12}>
        <Grid container spacing={gridSpacing}>
          <Grid item lg={3} sm={6} xs={12}>
            <ReportCard
              primary={(user?.role === 'CHEF' || user?.role === 'CHEF_SERVICE') ? stats.total_hebdo : "12"}
              secondary={(user?.role === 'CHEF' || user?.role === 'CHEF_SERVICE') ? "Dossiers (7j)" : "BEX en cours"}
              color={theme.palette.warning.main}
              footerData={(user?.role === 'CHEF' || user?.role === 'CHEF_SERVICE') ? "Nouveaux dossiers créés" : "Dossiers en attente"}
